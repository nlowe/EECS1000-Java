 ...
 * 
 * Numbers may be provided as command-line arguments, separated by spaces
 * If no numbers are provided, the user is asked for one
 * 
 * @author Nathan Lowe
 * 2014-12-3 3:07pm
 *
...
public class CCVerifyByChar
{
	...
	/**
	 * Checks one input and prints the result of validating the input with Luhn's algorithm
	 * @param cardNumber
	 */
	public static void checkAndPrint(String cardNumber){
		...
		System.out.println(cardNumber + " is " + (luhn(sanitizedNumber) ? "a VALID " : "an INVALID") + " card number");
	 ...
	 * @param input
	 * @return The sanitized input (Whitespace and non-numeric characters removed)
	 */
	 ...
	 * 	1) Sanitize the input number by stripping all whitespace and non-numeric characters
	 * 	2) Reverse the input string
	 *  3) For each individual digit
	 *  	3A) If the digit is in an odd position, double it.
	 *  		If the resultant number is greater than nine, instead store the sum
	 *  		of it's constituent digits
	 *  4) Sum each of the digits
	 *  5) Iff the sum ends in a zero, then it is a valid number.
	 * 
	 * @param cardNumber The card number to check
	 * @return Whether or not the card number is valid according to Luhn's algorithm
	 */
	public static boolean luhn(String cardNumber)
	{
		String reversed = new StringBuilder(sanitize(cardNumber)).reverse().toString();
	...
	/**
	 * Splits a string of numbers into its individual digits
	 * NOTE: This method will strip any non-numeric character from the input string
	 * 
